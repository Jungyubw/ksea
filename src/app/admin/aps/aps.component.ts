  ...
  ngOnInit() {
    ...
    if (!this.users || this.users.length === 0) {
      this.userService.retriveUsers().subscribe(u_data => {
        ...
        if (!this.apsdefs || this.apsdefs.length === 0) {
          this.userService.retriveApsDefs().subscribe(c_data => {
            this.apsdefs = <APS_DEF[]>c_data;
            this.userService.setApsDefs(this.apsdefs);
            this.reload();
